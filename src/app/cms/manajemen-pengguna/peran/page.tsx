...
import { Peran, PeranResponse } from "@/model/admin/manajemen-pengguna/Peran";
import { ApiResponse } from "@/model/ApiResponse";
import useSWR from "swr";
import { fetcherPepdas, fetcherSuperadmin } from "lib/fetcher";
import { deletePeran } from "./lib/action";
...
export default function PeranPage() {
  ...
  const swrKey = useMemo(() => {
    ...
    return `/peran?${params.toString()}`;
  }, [pageIndex, pageSize, searchBy, searchValue]);

  const { data: currentData, isLoading, mutate } = useSWR<ApiResponse<PeranResponse>>(swrKey, fetcherSuperadmin);

  const peranList: Peran[] = currentData?._embedded?.peranList ?? [];
  ...
  const table = useReactTable({
    ...
    getRowId: (row: Peran) => row.id.toString(),
  ...
  const handleDeleteSelected = async () => {
    try {
      ...
      await Promise.all(idsToDelete.map((id) => deletePeran(id)));
      await mutate();
      setSelectedRowIds({});
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex flex-col">
          <div className="flex items-center justify-between mb-2">
            <div>
              <Breadcrumbs items={[{ label: "Peran", href: "" }]} />
              <div className="flex items-center gap-2 text-secondary-green">
                <List />
                <h1 className="text-2xl font-bold ">Peran</h1>
              </div>
              <p className="text-sm text-base-gray">Informasi terkait data peran</p>
            </div>
            <Link href="peran/create">

...
import { KonfigurasiSistem, KonfigurasiSistemResponse } from "@/model/admin/konfigurasi-sistem/KonfigurasiSistem";
import { ApiResponse } from "@/model/ApiResponse";
import useSWR from "swr";
import { fetcherPepdas } from "lib/fetcher";
import { deleteKonfigurasiSistem } from "./lib/action";
...
export default function KonfigurasiSistemPage() {
  ...
  const { data: currentData, isLoading, mutate } = useSWR<ApiResponse<KonfigurasiSistemResponse>>(swrKey, fetcherPepdas);

  const konfigurasiSistemList: KonfigurasiSistem[] = currentData?._embedded?.konfigurasiSistemList ?? [];
  ...
  const handleDeleteSelected = async () => {
    try {
      ...
      await Promise.all(idsToDelete.map((id) => deleteKonfigurasiSistem(id)));
      await mutate();
      setSelectedRowIds({});
        ...
        <div className="flex flex-col">
          <div className="flex items-center justify-between mb-2">
            <div>
              ...
              <p className="text-sm text-base-gray">Informasi terkait data konfigurasi sistem</p>
            </div>
            <Link href="konfigurasi-sistem/create">

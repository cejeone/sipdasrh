  ...
  return (
    <div className="p-6 space-y-8">
      <div>
        ...
        <p className="text-sm text-gray-600 mb-4">Data jumlah bibit kayu dan HHBK untuk kegiatan sulaman dan pemupukan</p>
      </div>
      ...
      <div className="border rounded-lg p-4 space-y-4 shadow-sm">
        ...
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>
                <Checkbox
                  checked={selectedSulaman.length === sulamanData.length}
                  onCheckedChange={() => setSelectedSulaman(selectedSulaman.length === sulamanData.length ? [] : sulamanData.map((d) => d.id))}
                />
              </TableHead>
          ...
          <TableBody>
            {sulamanData.map((data, index) => (
              <TableRow key={data.id}>
                <TableCell>
                  <Checkbox checked={selectedSulaman.includes(data.id)} onCheckedChange={() => toggleSelect(data.id, "sulaman")} />
                </TableCell>
        ...
        <div className="text-sm text-gray-600 flex justify-between items-center">
          <span>
            {selectedSulaman.length} dari {sulamanData.length} baris dipilih
          </span>
      ...
      <div className="border rounded-lg p-4 space-y-4 shadow-sm">
        ...
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>
                <Checkbox
                  checked={selectedPemupukan.length === pemupukanData.length}
                  onCheckedChange={() => setSelectedPemupukan(selectedPemupukan.length === pemupukanData.length ? [] : pemupukanData.map((d) => d.id))}
                />
              </TableHead>
              <TableHead>No.</TableHead>
              <TableHead>Jenis</TableHead>
          ...
          <TableBody>
            {pemupukanData.map((data, index) => (
              <TableRow key={data.id}>
                <TableCell>
                  <Checkbox checked={selectedPemupukan.includes(data.id)} onCheckedChange={() => toggleSelect(data.id, "pemupukan")} />
                </TableCell>
                <TableCell>{index + 1}</TableCell>
                <TableCell>{data.jenis}</TableCell>
        ...
        <div className="text-sm text-gray-600 flex justify-between items-center">
          <span>
            {selectedPemupukan.length} dari {pemupukanData.length} baris dipilih
          </span>
          <div className="flex gap-2 items-center">
            <span>Baris per Halaman</span>
            <select className="border rounded px-2 py-1">

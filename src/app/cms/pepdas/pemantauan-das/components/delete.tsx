...
import { deletePemantauanDAS, detailPemantauanDAS } from "../lib/action";
import { toast } from "sonner";

interface Props {
  id: string;
  mutatePemantauanDAS: () => void;
}

export default function Delete({ id, mutatePemantauanDAS }: Props) {
  const handleDelete = async () => {
    try {
      await deletePemantauanDAS(id);
      mutatePemantauanDAS(); // Langsung panggil mutate

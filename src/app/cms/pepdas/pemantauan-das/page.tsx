...
import { PemantauanDAS, PemantauanDASResponse } from "@/model/pepdas/PemantauanDAS";
import { ApiResponse } from "@/model/ApiResponse";
import useSWR from "swr";
import { fetcherPepdas } from "lib/fetcher";
import { deletePemantauanDAS } from "./lib/action";
import { toast } from "sonner";

export default function PemantauanDASPage() {
  ...
  const swrKey = useMemo(() => {
    ...
    return `/pemantauan-das?${params.toString()}`;
  }, [pageIndex, pageSize, searchBy, searchValue]);

  const { data: currentData, isLoading, mutate } = useSWR<ApiResponse<PemantauanDASResponse>>(swrKey, fetcherPepdas);

  const pemantauanDasList: PemantauanDAS[] = currentData?._embedded?.pemantauanDasList ?? [];
  ...
  const handleDeleteSelected = async () => {
    try {
      ...
      await Promise.all(idsToDelete.map((id) => deletePemantauanDAS(id)));
      await mutate();
      setSelectedRowIds({});
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex flex-col">
          <div className="flex items-center justify-between mb-2">
            <div>
              <Breadcrumbs items={[{ label: "PEPDAS", href: "" }, { label: "Pemantauan DAS" }]} />
            ...
            <Link href="pemantauan-das/create">

...
import { Dokumen, DokumenResponse } from "@/model/pepdas/Dokumen";
...
export default function DokumenPage() {
  ...
  const { data: currentData, isLoading, mutate } = useSWR<ApiResponse<DokumenResponse>>(swrKey, fetcherPepdas);
  ...
  const handleDeleteSelected = async () => {
    try {
      ...
      await Promise.all(idsToDelete.map((id) => deleteDokumen(id)));
      await mutate();
      setSelectedRowIds({});
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="flex flex-col">
        <div className="flex items-center justify-between mb-2">
          <div>
            <Breadcrumbs items={[{ label: "PEPDAS", href: "" }, { label: "Dokumen" }]} />
                ...
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button variant="destructive" className="text-white text-left">
                      <Trash2Icon className="mr-2" /> Hapus
                    </Button>
                  </AlertDialogTrigger>
                  <AlertDialogContent>
                    <AlertDialogHeader>

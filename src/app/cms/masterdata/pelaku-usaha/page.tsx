...
import { PelakuUsaha, PelakuUsahaResponse } from "@/model/admin/masterdata/PelakuUsaha";
import { ApiResponse } from "@/model/ApiResponse";
import useSWR from "swr";
import { fetcherSuperadmin } from "lib/fetcher";
import { deletePelakuUsaha } from "./lib/action";
import { toast } from "sonner";

export default function PelakuUsahaPage() {
  ...
  const table = useReactTable({
    ...
    getRowId: (row: PelakuUsaha) => row.id.toString(),
  ...
  const handleDeleteSelected = async () => {
    try {
      ...
      await Promise.all(idsToDelete.map((id) => deletePelakuUsaha(id)));
      await mutate();
      setSelectedRowIds({});
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex flex-col">
          <div className="flex items-center justify-between mb-2">
            <div>
              <Breadcrumbs items={[{ label: "Masterdata" }, { label: "Pelaku Usaha" }]} />
              <div className="flex items-center gap-2 text-secondary-green">
                <Building2 />
                <h1 className="text-2xl font-bold ">Pelaku Usaha</h1>
              </div>
              <p className="text-sm text-base-gray">Informasi terkait data pelaku usaha</p>
            </div>
            <Link href="pelaku-usaha/create">

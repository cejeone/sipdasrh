...
import { ChevronsLeft, ChevronsRight, ChevronLeft, ChevronRight, ChevronDown, Settings2Icon, Plus, Trash2Icon, UserSquare2 } from "lucide-react";
...
import { KelompokMasyarakat, KelompokMasyarakatResponse } from "@/model/admin/masterdata/KelompokMasyarakat";
import { ApiResponse } from "@/model/ApiResponse";
import useSWR from "swr";
import { fetcherSuperadmin } from "lib/fetcher";
import { deleteKelompokMasyarakat } from "./lib/action";
import { toast } from "sonner";

export default function KelompokMasyarakatPage() {
  ...
  const swrKey = useMemo(() => {
    ...
    return `/kelompok-masyarakat?${params.toString()}`;
  }, [pageIndex, pageSize, searchBy, searchValue]);

  const { data: currentData, isLoading, mutate } = useSWR<ApiResponse<KelompokMasyarakatResponse>>(swrKey, fetcherSuperadmin);

  const kelompokMasyarakatList: KelompokMasyarakat[] = currentData?._embedded?.kelompokMasyarakatList ?? [];
  ...
  const table = useReactTable({
    ...
    getRowId: (row: KelompokMasyarakat) => row.id.toString(),
  ...
  const handleDeleteSelected = async () => {
    try {
      ...
      await Promise.all(idsToDelete.map((id) => deleteKelompokMasyarakat(id)));
      await mutate();
      setSelectedRowIds({});
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex flex-col">
          <div className="flex items-center justify-between mb-2">
            <div>
              <Breadcrumbs items={[{ label: "Masterdata" }, { label: "Kelompok Masyarakat" }]} />

...
import { deleteKabupaten } from "./lib/action";
...
export default function KabupatenPage() {
  ...
  const swrKey = useMemo(() => {
    ...
    return `/kabupaten-kota?${params.toString()}`;
  }, [pageIndex, pageSize, searchBy, searchValue]);

  const { data: currentData, isLoading, mutate } = useSWR<ApiResponse<KabupatenKotaResponse>>(swrKey, fetcherSuperadmin);

  const kabupatenKotaList: KabupatenKota[] = currentData?._embedded?.kabupatenKotaList ?? [];
  ...
  const table = useReactTable({
    ...
    // getRowId: (row) => row.id,
    getRowId: (row: KabupatenKota) => row.id.toString(),
  ...
  const handleDeleteSelected = async () => {
    try {
      ...
      await Promise.all(idsToDelete.map((id) => deleteKabupaten(id)));
      await mutate();
      setSelectedRowIds({});
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex flex-col">
          <div className="flex items-center justify-between mb-2">
            <div>
              <Breadcrumbs items={[{ label: "Masterdata", href: "" }, { label: "Struktur Wilayah", href: "" }, { label: "Kabupaten" }]} />
              <div className="flex items-center gap-2 text-secondary-green">
                <List />
                <h1 className="text-2xl font-bold ">Kabupaten / Kota</h1>
              </div>
              <p className="text-sm text-base-gray">Informasi terkait data kabupaten / kota</p>
            </div>
            <Link href="kabupaten/create">

...
import { ChevronsLeft, ChevronsRight, ChevronLeft, ChevronRight, ChevronDown, Settings2Icon, Plus, Trash2Icon, Link2, List } from "lucide-react";
...
import { deleteProvinsi } from "./lib/action";
...
export default function ProvinsiPage() {
  ...
  const swrKey = useMemo(() => {
    ...
    return `/provinsi?${params.toString()}`;
  }, [pageIndex, pageSize, searchBy, searchValue]);

  const { data: currentData, isLoading, mutate } = useSWR<ApiResponse<ProvinsiResponse>>(swrKey, fetcherSuperadmin);

  const provinsiList: Provinsi[] = currentData?._embedded?.provinsiList ?? [];
  ...
  const table = useReactTable({
    ...
    // getRowId: (row) => row.id,
    getRowId: (row: Provinsi) => row.id.toString(),
  ...
  const handleDeleteSelected = async () => {
    try {
      ...
      await Promise.all(idsToDelete.map((id) => deleteProvinsi(id)));
      await mutate();
      setSelectedRowIds({});
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex flex-col">
          <div className="flex items-center justify-between mb-2">
            <div>
              <Breadcrumbs items={[{ label: "Masterdata", href: "" }, { label: "Struktur Wilayah", href: "" }, { label: "Provinsi" }]} />
              <div className="flex items-center gap-2 text-secondary-green">
                <List />
                <h1 className="text-2xl font-bold ">Provinsi</h1>
              </div>
              <p className="text-sm text-base-gray">Informasi terkait data provinsi</p>
            </div>
            <Link href="provinsi/create">

...
import { deleteKecamatan } from "./lib/action";
...
export default function KecamatanPage() {
  ...
  const swrKey = useMemo(() => {
    ...
    return `/kecamatan?${params.toString()}`;
  }, [pageIndex, pageSize, searchBy, searchValue]);

  const { data: currentData, isLoading, mutate } = useSWR<ApiResponse<KecamatanResponse>>(swrKey, fetcherSuperadmin);

  const kecamatanList: Kecamatan[] = currentData?._embedded?.kecamatanList ?? [];
  ...
  const table = useReactTable({
    ...
    // getRowId: (row) => row.id,
    getRowId: (row: Kecamatan) => row.id.toString(),
  ...
  const handleDeleteSelected = async () => {
    try {
      ...
      await Promise.all(idsToDelete.map((id) => deleteKecamatan(id)));
      await mutate();
      setSelectedRowIds({});
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex flex-col">
          <div className="flex items-center justify-between mb-2">
            <div>
              <Breadcrumbs items={[{ label: "Masterdata", href: "" }, { label: "Struktur Wilayah", href: "" }, { label: "Kecamatan" }]} />
              <div className="flex items-center gap-2 text-secondary-green">
                <List />
                <h1 className="text-2xl font-bold ">Kecamatan</h1>
              </div>
              <p className="text-sm text-base-gray">Informasi terkait data kecamatan</p>
            </div>
            <Link href="kecamatan/create">

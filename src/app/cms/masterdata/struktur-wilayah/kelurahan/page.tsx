...
import { deleteKelurahanDesa } from "./lib/action";
...
export default function KecamatanPage() {
  ...
  const swrKey = useMemo(() => {
    ...
    return `/kelurahan-desa?${params.toString()}`;
  }, [pageIndex, pageSize, searchBy, searchValue]);

  const { data: currentData, isLoading, mutate } = useSWR<ApiResponse<KelurahanDesaResponse>>(swrKey, fetcherSuperadmin);

  const kelurahanDesaList: KelurahanDesa[] = currentData?._embedded?.kelurahanDesaList ?? [];
  ...
  const table = useReactTable({
    ...
    // getRowId: (row) => row.id,
    getRowId: (row: KelurahanDesa) => row.id.toString(),
  ...
  const handleDeleteSelected = async () => {
    try {
      ...
      await Promise.all(idsToDelete.map((id) => deleteKelurahanDesa(id)));
      await mutate();
      setSelectedRowIds({});
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex flex-col">
          <div className="flex items-center justify-between mb-2">
            <div>
              <Breadcrumbs items={[{ label: "Masterdata", href: "" }, { label: "Struktur Wilayah", href: "" }, { label: "Kelurahan" }]} />
              <div className="flex items-center gap-2 text-secondary-green">
                <List />
                <h1 className="text-2xl font-bold ">Kelurahan / Desa</h1>
              </div>
              <p className="text-sm text-base-gray">Informasi terkait data kelurahan / desa</p>
            </div>
            <Link href="kelurahan/create">

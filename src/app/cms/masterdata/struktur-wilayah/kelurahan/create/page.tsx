...
const CreateKelurahanPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await AxiosInstance.get<ApiResponse<KecamatanResponse>>("/kecamatan");
        const responseData = response.data;

        setDataKecamatan(responseData._embedded?.kecamatanList);
      ...
      <div className="flex flex-col">
        <div className="flex items-center justify-between mb-2">
          <div>
            ...
            <p className="text-sm text-base-gray">Form untuk membuat data kelurahan / desa</p>
          </div>
        ...
        <Card className="border border-border p-4 mb-2 bg-card">
          <CardContent className="grid grid-cols-1 lg:grid-cols-12 gap-4 p-0">
            ...
            <div className="col-span-12 lg:col-span-6">
              <Card>
                <FormKabupatenPage type="ADD" kecamatanList={dataKecamatan} ref={formRef} />
              </Card>
            </div>
          </CardContent>
        </Card>
...
export default CreateKelurahanPage;

...
import { deleteBpdas } from "./lib/action";
import { toast } from "sonner";

export default function BpdasPage() {
  ...
  const swrKey = useMemo(() => {
    ...
    return `/bpdas?${params.toString()}`;
  }, [pageIndex, pageSize, searchBy, searchValue]);

  const { data: currentData, isLoading, mutate } = useSWR<ApiResponse<BpdasResponse>>(swrKey, fetcherSuperadmin);

  const bpdasList: Bpdas[] = currentData?._embedded?.bpdasList ?? [];
  ...
  const table = useReactTable({
    ...
    getRowId: (row: Bpdas) => row.id.toString(),
  ...
  const handleDeleteSelected = async () => {
    try {
      ...
      await Promise.all(idsToDelete.map((id) => deleteBpdas(id)));
      await mutate();
      setSelectedRowIds({});
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex flex-col">
          <div className="flex items-center justify-between mb-2">
            <div>
              <Breadcrumbs items={[{ label: "Organisasi" }, { label: "BPDAS" }]} />

...
import { Bpth, BpthResponse } from "@/model/admin/organisasi/Bpth";
import { ApiResponse } from "@/model/ApiResponse";
import useSWR from "swr";
import { fetcherPepdas, fetcherSuperadmin } from "lib/fetcher";
import { deleteBpth } from "./lib/action";
...
export default function BpthPage() {
  ...
  const swrKey = useMemo(() => {
    ...
    return `/bpth?${params.toString()}`;
  }, [pageIndex, pageSize, searchBy, searchValue]);

  const { data: currentData, isLoading, mutate } = useSWR<ApiResponse<BpthResponse>>(swrKey, fetcherSuperadmin);

  const bpthList: Bpth[] = currentData?._embedded?.bpthList ?? [];
  ...
  const table = useReactTable({
    ...
    getRowId: (row: Bpth) => row.id.toString(),
  ...
  const handleDeleteSelected = async () => {
    try {
      ...
      await Promise.all(idsToDelete.map((id) => deleteBpth(id)));
      await mutate();
      setSelectedRowIds({});
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex flex-col">
          <div className="flex items-center justify-between mb-2">
            <div>
              <Breadcrumbs items={[{ label: "Organisasi" }, { label: "BPTH" }]} />
              <div className="flex items-center gap-2 text-secondary-green">
                <Building />
                <h1 className="text-2xl font-bold ">BPTH</h1>
              </div>
              <p className="text-sm text-base-gray">Informasi terkait data BPTH</p>
            </div>
            <Link href="bpth/create">
